...
class ReclaimModuleApi: ReclaimModuleApiProtocol {
  ...
  func setOverrides(provider providerArg: ClientProviderInformationOverride?, feature featureArg: ClientFeatureOverrides?, logConsumer logConsumerArg: ClientLogConsumerOverride?, sessionManagement sessionManagementArg: ClientReclaimSessionManagementOverride?, appInfo appInfoArg: ClientReclaimAppInfoOverride?, completion: @escaping (Result<Void, PigeonError>) -> Void) {
    ...
    let channel = FlutterBasicMessageChannel(name: channelName, binaryMessenger: binaryMessenger, codec: codec)
    channel.sendMessage([providerArg, featureArg, logConsumerArg, sessionManagementArg, appInfoArg] as [Any?]) { response in
      guard let listResponse = response as? [Any?] else {
        completion(.failure(createConnectionError(withChannelName: channelName)))
        return
      }
  ...
  static func setUp(binaryMessenger: FlutterBinaryMessenger, api: ReclaimApi?, messageChannelSuffix: String = "") {
    ...
    if let api = api {
      logSessionChannel.setMessageHandler { message, reply in
        ...
        do {
          try api.logSession(appId: appIdArg, providerId: providerIdArg, sessionId: sessionIdArg, logType: logTypeArg)
          reply(wrapResult(nil))
        } catch {
          reply(wrapError(error))
        }
